    ...
    <main className="mx-auto flex min-h-dvh w-full max-w-6xl flex-col gap-5 px-6 py-8">
      <header className="flex flex-wrap items-center justify-between gap-4 border-b border-slate-200 pb-5">
        <div>
          ...
          <p className="mt-0.5 text-sm text-slate-400">Catalogue produits & services.</p>
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
              ...
              {filtered.map((r) => (
                <tr key={r.id} className="text-slate-900">
                  <td className="px-5 py-3">
                    ...
                    {r.description ? <div className="mt-1 text-xs text-slate-400 line-clamp-2">{r.description}</div> : null}
                  </td>
                  <td className="px-5 py-3">{Number(r.unitPriceHt || 0).toFixed(2)} €</td>
                  <td className="px-5 py-3">
                    <span className={['inline-flex rounded-full px-3 py-1 text-xs font-medium', r.isActive ? 'bg-blue-50 text-blue-700' : 'bg-slate-100 text-slate-700'].join(' ')}>
                      {r.isActive ? 'Oui' : 'Non'}
                    </span>
                  </td>
                  <td className="px-5 py-3 text-right">
                    <button className="rounded-xl px-3 py-2 text-xs font-semibold text-slate-900 transition hover:bg-slate-100" onClick={() => startEdit(r)} type="button">
                      Modifier
                    </button>
        ...
        <div className="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="text-sm font-semibold text-slate-900">{editing ? 'Modifier un service' : 'Créer un service'}</div>
          <div className="mt-4 grid gap-3">
            <label className="grid gap-1 text-xs font-medium text-slate-700">
              Nom
              <input className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500" onChange={(e) => setForm((p) => ({ ...p, name: e.target.value }))} value={form.name} />
            </label>
            <label className="grid gap-1 text-xs font-medium text-slate-700">
              Description
              <textarea className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500" rows={4} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} value={form.description} />
            </label>
            <div className="grid gap-3 sm:grid-cols-2">
              <label className="grid gap-1 text-xs font-medium text-slate-700">
                Unité
                <input className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500" onChange={(e) => setForm((p) => ({ ...p, unit: e.target.value }))} value={form.unit} />
              </label>
              <label className="grid gap-1 text-xs font-medium text-slate-700">
                Prix HT
                <input className="rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:border-blue-500" inputMode="decimal" onChange={(e) => setForm((p) => ({ ...p, unitPriceHt: e.target.value }))} value={form.unitPriceHt} />
              </label>
            </div>
            ...
            <button className="rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white transition hover:bg-blue-700" onClick={save} type="button">
              Enregistrer
            </button>
          </div>
        </div>
